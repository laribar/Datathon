...
def setup_paths():
    ...
    os.makedirs(EMB_DIR, exist_ok=True) # Usa o objeto Path já definido
    return base_path
...
def _save_embeddings(npy_path: Path, meta_path: Path, embs: np.ndarray, meta: dict) -> None:
    ...
    try:
        ...
        st.warning(f"⚠️ Falha ao salvar cache no disco ({npy_path.name}). Isso é normal em alguns deploys.", icon="⚠️") 
...
def get_or_build_embeddings(df: pd.DataFrame, text_col: str, model_dir_or_flag: str) -> np.ndarray:
    ...
    def _is_valid(embs: np.ndarray | None, meta: dict, df: pd.DataFrame) -> bool:
        ...
        if model_dir_or_flag == "FORCE_REBUILD": return False 
    ...
    st.warning(f"⏳ Reconstruindo embeddings para '{text_col}' (Não há cache válido). Isso pode levar alguns minutos...", icon="⏳")
...
def load_model(model_path: str):
    ...
    try:
        ...
        try:
            ...
            st.warning(f"⚠️ Modelo padrão do Hugging Face carregado como fallback.", icon="⚠️")
            return model
        except Exception:
...
def generate_xgb_features(vaga_emb: np.ndarray, cv_embs: np.ndarray) -> np.ndarray:
    ...
    features = np.hstack([
        vaga_embs_repeated, 
        cv_embs, 
        np.abs(vaga_embs_repeated - cv_embs), 
        vaga_embs_repeated * cv_embs 
    ])
...
with st.sidebar:
    ...
    # Informações de Memória (movidas para DENTRO do bloco Streamlit)
...
with tab_bases:
    ...
    if not cdf.empty:
        with st.expander(f"Candidatos ({len(cdf)} registros)", expanded=True):
            ...
            st.caption(f"Colunas combinadas para embedding: **cv_text**")
            st.download_button("Baixar CSV Candidatos", cdf.to_csv(index=False).encode("utf-8"), file_name="candidatos_processado.csv", mime="text/csv", key="dl_cand")
            
    if not vdf.empty:
        with st.expander(f"Vagas ({len(vdf)} registros)", expanded=True):
            st.dataframe(vdf, use_container_width=True)
            st.caption(f"Colunas combinadas para embedding: **vaga_text**")
            st.download_button("Baixar CSV Vagas", vdf.to_csv(index=False).encode("utf-8"), file_name="vagas_processado.csv", mime="text/csv", key="dl_vaga")